...
const STORAGE_KEY = "taskbar-job-preparation";
...
function loadData() {
  try {
    ...
    console.error("Failed to load Job Preparation data:", error);
    return DEFAULT_DATA;
  }
}
...
export default function JobPreparation() {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent(JOB_PREPARATION_UPDATED_EVENT));
  }, [data]);

  const completedSkills = useMemo(
    () => data.skills.filter((skill) => skill.completed).length,
    [data.skills]
  );

  const skillProgress =
    data.skills.length > 0
      ? Math.round((completedSkills / data.skills.length) * 100)
      : 0;
  ...
  const overallProgress = Math.round((skillProgress + taskProgress) / 2);
    ...
    <div className="job-preparation-page">
      <div className="job-preparation-header">
        <div className="job-preparation-title-area">
          ...
          <p>
            Track your career preparation and job-readiness progress.
          </p>
        </div>
        ...
        {!editingGoal ? (
          <div className="job-preparation-goal-content">
            <div>
              ...
              <h3>
                {data.careerGoal || "Set your career goal"}
              </h3>

              <p>
                Target Role:{" "}
                <strong>{data.targetRole || "Not set"}</strong>
                {data.targetDate && (
                  <>
                    {" "}
        ...
        <section className="job-preparation-card">
          <div className="job-preparation-section-header">
            <div>
              <h2>Skills</h2>
              <p>
                Track the technologies you need for your target role.
              </p>
            </div>
        ...
        <section className="job-preparation-card">
          <div className="job-preparation-section-header">
            <div>
              ...
              <p>
                Small actions that move you closer to a job.
              </p>
            </div>
      ...
      <section className="job-preparation-card job-preparation-notes">
        <div className="job-preparation-section-header">
          <div>
            ...
            <p>
              Keep important career preparation notes here.
            </p>
          </div>

          <Save size={18} className="job-preparation-muted-icon" />
        </div>
  ...
  return (
    <div className="job-preparation-progress-card">
      <div className="job-preparation-progress-card-label">
        <span>{title}</span>
        <strong>{percentage ? `${progress}%` : `${completed}/${total}`}</strong>
      </div>
